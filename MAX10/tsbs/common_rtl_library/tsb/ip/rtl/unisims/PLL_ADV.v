// $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/verunilibs/data/rainier/PLL_ADV.v,v 1.28.4.7 2007/09/14 22:02:36 yanx Exp $
///////////////////////////////////////////////////////////////////////////////
...
//*** Clocks MUX
    ...
    assign rel_o_mux_clk_tmp = rel_o_mux_sel ? CLKIN1 : CLKFBIN;
    assign rel_o_mux_clk = (pmcd_mode) ? rel_o_mux_clk_tmp : 0;
    assign clka1_in = (pmcd_mode) ? CLKIN1 : 0;
    assign clkb1_in = (pmcd_mode) ? CLKFBIN : 0;
...
always @(CLKINSEL ) 
 if (pmcd_mode != 1) begin
  ...
  period_clkin =  (CLKINSEL) ? CLKIN1_PERIOD : CLKIN2_PERIOD;
 ...
 assign  init_trig = 1;
  ...
  assign clkpll_tmp = (CLKINSEL) ? CLKIN1 : CLKIN2;
  assign clkpll = (pmcd_mode) ? 0 : clkpll_tmp;

  assign orig_rst_in =  RST; 
...
always @(RST )
  if (RST==1)
     rst_edge = $time;
  else if (RST==0 && rst_edge > 1) begin
...
//
  ...
  assign DO = dr_sram[DADDR_lat];

always @(posedge DCLK or posedge GSR)
  if (GSR == 1) begin
       drp_lock <= 0;
    end
  else begin
    if (DEN == 1) begin
        valid_daddr = addr_is_valid(DADDR);
        if (drp_lock == 1) begin
        ...
        if (valid_daddr && ( DADDR == 5'b00110 || DADDR == 5'b00001 || DADDR == 5'b00000 ||
                         (DADDR >= 5'b01100 && DADDR <= 5'b11100 && DADDR != 5'b10000 && 
                                  DADDR != 5'b10001 && DADDR != 5'b11000 ))) begin
              end
        else begin
                  $display(" Warning : Address DADDR=%b is unsupported at PLL_ADV instance %m at time %t.  ",  DADDR, $time);
        end

        if (DWE == 1) begin          // write process
          if (RST == 1) begin
             if (valid_daddr && ( DADDR == 5'b00110 || DADDR == 5'b00001 || DADDR == 5'b00000 ||
                                  (DADDR >= 5'b01100 && DADDR <= 5'b11100 && DADDR != 5'b10000 && 
                                  DADDR != 5'b10001 && DADDR != 5'b11000 ))) begin
             ...
             if (DADDR == 5'b11100) 
                 clkout_delay_para_drp (clkout0_dly, clk0_nocnt, clk0_edge, DI, DADDR);

             if (DADDR == 5'b11011)
                 clkout_hl_para_drp (clk0_lt, clk0_ht, clk0pm_sel, DI, DADDR);

             if (DADDR == 5'b11010) 
                 clkout_delay_para_drp (clkout1_dly, clk1_nocnt, clk1_edge, DI, DADDR);

             if (DADDR == 5'b11001)
                 clkout_hl_para_drp (clk1_lt, clk1_ht, clk1pm_sel, DI, DADDR);

             if (DADDR == 5'b10111) 
                 clkout_delay_para_drp (clkout2_dly, clk2_nocnt, clk2_edge, DI, DADDR);

             if (DADDR == 5'b10110)
                 clkout_hl_para_drp (clk2_lt, clk2_ht, clk2pm_sel, DI, DADDR);

             if (DADDR == 5'b10101) 
                 clkout_delay_para_drp (clkout3_dly, clk3_nocnt, clk3_edge, DI, DADDR);

             if (DADDR == 5'b10100)
                 clkout_hl_para_drp (clk3_lt, clk3_ht, clk3pm_sel, DI, DADDR);

             if (DADDR == 5'b10011) 
                 clkout_delay_para_drp (clkout4_dly, clk4_nocnt, clk4_edge, DI, DADDR);

             if (DADDR == 5'b10010)
                 clkout_hl_para_drp (clk4_lt, clk4_ht, clk4pm_sel, DI, DADDR);

             if (DADDR == 5'b01111) 
                 clkout_delay_para_drp (clkout5_dly, clk5_nocnt, clk5_edge, DI, DADDR);

             if (DADDR == 5'b01110)
                 clkout_hl_para_drp (clk5_lt, clk5_ht, clk5pm_sel, DI, DADDR);

             if (DADDR == 5'b01101) 
                 clkout_delay_para_drp (clkfbm1_dly, clkfbm1_nocnt, clkfbm1_edge, DI, DADDR);

             if (DADDR == 5'b01100)
                 clkout_hl_para_drp (clkfbm1_lt, clkfbm1_ht, clkfbm1pm_sel, DI, DADDR);

             if (DADDR == 5'b00110) begin
                 clkind_lt <= DI[5:0];
                 clkind_ht <= DI[11:6];
                 if ( DI[5:0] == 6'b0 && DI[11:6] == 6'b0 )
                     clkind_div <= 8'b10000000;
                 else if (DI[5:0] == 6'b0 && DI[11:6] != 6'b0 )
                     clkind_div <= 64 + DI[11:6];
                 else if (DI[5:0] == 6'b0 && DI[11:6] != 6'b0 )
                     clkind_div <= 64 + DI[5:0];
                 else
                      clkind_div <= DI[5:0] + DI[11:6];
                 clkind_nocnt <= DI[12];
                 clkind_edge <= DI[13];
             end
...
always @(posedge CLKFBIN or posedge rst_in1 ) 
  if (rst_in1) begin
    fb_delay  <= 0;
    fb_delay_found_tmp <= 0;
  end
...
always @(CLKFBIN)
  if (pmcd_mode != 1)
     CLKFBIN_dly <= #clkfb_dly_t CLKFBIN;
  else 
      CLKFBIN_dly = 0;
...
always @( posedge CLKFBIN_dly or negedge CLKFBIN or posedge rst_in)
  if (rst_in || CLKFBIN == 0) begin
      clkfbstop_cnt_p = 0;
      clkfb_stopped_p = 0;
  end
  else   
    if (fb_delay_found && pll_locked_tmp2) begin 
      if (CLKFBIN && clkpll_jitter_unlock == 0)
...
always @( posedge CLKFBIN_dly or posedge CLKFBIN or posedge rst_in)
  if (rst_in==1 || CLKFBIN == 1) begin
      clkfbstop_cnt_n = 0;
      clkfb_stopped_n = 0;
  end
  else   
    if (fb_delay_found && pll_locked_tmp2) begin
      if (CLKFBIN==0 && clkpll_jitter_unlock == 0)
...
begin
 ...
 if (clk_ps < 0.0)
  ...
  if (clkout_dly_tmp > 63) begin
    $display(" Warning : Attribute %s on instance %m is set to %f. Required phase shifting can not be reached since it is over the maximum phase shifting ability of PLL_ADV", clk_ps_name, clk_ps);
...
begin
    if (di_in[12] != 1) begin
         $display(" Error : PLL_ADV on instance %m input DI is %d at address DADDR=%b at time %t. The bit 12 need to be set to 1 .", di_in, daddr_in, $time); 
         $finish;
    end
    if ( di_in[5:0] == 6'b0)
       clk_lt = 7'b1000000;
    ...
    if (di_in[11:6] == 6'b0)
      clk_ht = 7'b1000000;
       ...
       clk_ht = { 1'b0, di_in[11:6]};
    clkpm_sel = di_in[15:13];
end
endtask
