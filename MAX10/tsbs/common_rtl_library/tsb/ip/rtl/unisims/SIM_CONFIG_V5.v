...
//  tri0 GSR, GTS, GWE;
  ...
  always @( posedge rst_en or posedge init_rst or posedge prog_b_in or posedge iprog_b )
   if (init_rst)
       init_b_out <= 0;
   else if (rst_en) begin
     if (prog_b_in == 0 )
         init_b_out <= 0;
     else
   ...
   always @(posedge gsr_cmd_out or negedge  rw_en)
    ...
    always @(posedge cclk_in or negedge rst_intl )
      if (rst_intl == 0) begin
         ...
         if (st_state == done_cycle_reg) begin
            if (DONE == 1 || done_pin_drv == 1) begin
                  done_o <= 1'b1;
             end
             else begin
                 done_o <= 1'bz;
