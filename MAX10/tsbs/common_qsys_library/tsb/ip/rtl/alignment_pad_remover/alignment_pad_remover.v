...
assign asi_snk0_ready   =   ((state == INITIAL)) ? (1'b1) :
                            ...
                            ((state == PAD_REMOVED) && (interlock_cycle)) ? (1'b1) :
                            ((state == AT_EOP) && (interlock_cycle)) ? (1'b1) :
                            ((state == AT_EOP_PLUS_1) && (interlock_cycle)) ? (1'b1) :
                            (1'b0);
...
assign pipe_src0_data   =   ({sink_data_1[15:0], sink_data_0[31:16]});
...
begin
    if(csi_clock_reset) begin
        ...
        case(state)
            INITIAL: begin
                if((sink_cycle) && (sink_sop_0)) begin
                    state <= AT_SOP;
                end
            end
            AT_SOP: begin
            ...
            AT_EOP_PLUS_1: begin
                if((source_cycle)) begin
                    state <= ((interlock_cycle) && (sink_sop_0)) ? (AT_SOP) : (INITIAL);
                end
            end
...
// output pipeline
//
alignment_pad_remover_1stage_pipeline #( .PAYLOAD_WIDTH( 36 ) ) outpipe (
    .clk            (csi_clock_clk ),
...
//  --------------------------------------------------------------------------------
module alignment_pad_remover_1stage_pipeline  
