...
#(
parameter [15:0] raw_packet_length_in_bytes = 256
(
    // clock interface
    input           csi_clock_clk,
...
localparam [4:0] STATE_SOP_ERR  = 5'd17;
...
// misc computations
//
assign udp_length           = raw_packet_length_in_bytes + 8;
assign first_two_bytes      = raw_packet_length_in_bytes;
...
assign asi_snk0_ready = (state == IDLE)         ? (1'b0) :
                        ...
                        ((state >= STATE_PW_11) && (state <= STATE_PW_N))   ? (pipe_src0_ready & asi_snk0_valid & !sink_eop) : //modification from udp_payload_inserter: keep sink_data as is, since we will be inserting fixed length for packet length
                        (1'b0);
...
begin
    ...
    begin
        ...
        begin
            ...
            begin
                state <= STATE_PW_11;               
                //modification from udp_payload_inserter: keep sink_data as is, since we will be inserting fixed length for packet length
            end
        end
        STATE_PW_11:
        begin
...
// output Pipeline
//
griffin_udp_payload_inserter_1stage_pipeline #( .PAYLOAD_WIDTH( 36 ) ) outpipe (
    .clk            (csi_clock_clk ),
...
//  --------------------------------------------------------------------------------
module griffin_udp_payload_inserter_1stage_pipeline  
