...
// STD RX = 5
// STD TX = 5
// (1024 != 0) and (1 == 0) and (0 != 0) and (5 != 0)
...
module  five_lane_seriallite_slite2_top /* vx2 no_prefix */  (
...
input[160 - 1:0] rx_parallel_data_out;
input rx_coreclk;
input[20 - 1:0] rx_ctrldetect;
input[20 - 1:0] stat_rr_pattdet;
input[20 - 1:0] err_rr_disp;
output[5 - 1:0] flip_polarity;
output rrefclk;
output stat_rr_link;
output[20 - 1:0] err_rr_8berrdet;
input ctrl_rr_enadscram;
output[160 - 1:0] tx_parallel_data_in;
output[20 - 1:0] tx_ctrlenable;
input tx_coreclk;
output tx_coreclock;
...
input mreset_n;
output rxrdp_ena;
output[160 - 1:0] rxrdp_dat;
input txrdp_ena;
output txrdp_dav;
input[160 - 1:0] txrdp_dat;
output[5 - 1:0] err_rr_pol_rev_required;
...
// From RX Core:
wire  [160 - 1:0] rx_parallel_data_out  /*   Serial Lite differential receive data bus.Bus carries the receiver data output.  */
...
wire  [20 - 1:0] err_rr_8berrdet  /*  Serial Lite receiver's 8b10b Error detect signal (1,2 or 4 bits)          */
...
wire  [160 - 1:0] tx_parallel_data_in  /*   Serial Lite transmit data bus to the transceiver. Bus carries packets cells or in-band control words  */
...
wire  [20 - 1:0] tx_ctrlenable  /*   Serial Lite transmit control enable to the transceiver.  */
...
wire  [160 - 1:0] rxrdp_dat  /*  User Data bits. In streaming mode, this signal is on the rrefclk domain UNLESS clock compensation is used, in which case the signal is on the TREFCLK domain */
...
wire  [160 - 1:0] txrdp_dat  /*  User Data bits.  */
/* vx2 port_info <desc scope="internal"/> <grpmember grpid=       "aif"        />     <desc> User Data bits. </desc> */;
// From RX Core:
wire  [5 - 1:0] err_rr_pol_rev_required  /*  Catastrophic error. Polarity on input GXB lines is reversed, and core can't change polarity. Core is unable to function.  */
...
wire  rcvd_clk2 ;
wire  rcvd_clk3 ;
wire  rcvd_clk4  ; // trefclk available for all configurations.
wire  tx_reset_n ;
wire  tc_reset_n  ;
wire  tc_ll_reset_n  ;
// Stratix II GX TX uses coreclock. Sync the reset to this domain as well.
wire  [5 - 1:0] tx_clkout  ;
assign tx_coreclock = tx_clkout[0];

assign tx_clkout[0] = tx_coreclk;// call to module reset_syncer;
/*CALL*/

 five_lane_seriallite_reset_syncer /* vx2 no_prefix */   tx_coreclock_reset_syncer(.clk(tx_coreclock),
// input
...
/*CALL*/
 five_lane_seriallite_reset_syncer /* vx2 no_prefix */   rrefclk_reset_syncer(.clk(rrefclk),
// input
.reset_in(mreset_n), // input
.sync_signal_in(1'b1), // input
...
/////////////////////////////////////////////////
wire[160 - 1:0] pcs_lsm_data; // input for module slite2_phy
wire[20 - 1:0] pcs_lsm_ctrl; // input for module slite2_phy
wire[20 - 1:0] pcs_lsm_val; // input for module slite2_phy
assign pcs_lsm_val = 20'd1;// Always Valid
// outputs for module slite2_phy;


wire[160 - 1:0] lsm_pdf_data; // output for module slite2_phy
wire[20 - 1:0] lsm_pdf_ctrl; // output for module slite2_phy
wire[20 - 1:0] lsm_pdf_val; // output for module slite2_phy
wire[160 - 1:0] lsm_pcs_data; // output for module slite2_phy
wire[20 - 1:0] lsm_pcs_ctrl; // output for module slite2_phy
wire[20 - 1:0] lsm_pcs_val;
// output for module slite2_phy
///////////////////////////////////////////////
/////////// Reset Logic for GXB ///////////////
///////////////////////////////////////////////
// Put GXB Here.
wire[2 - 1:0] pll_tx_locked;
wire stat_inval_patdet;
// Invalid Pattern Detect signal received. Debug signal.
wire[2 - 1:0] pll_locked;
// State machine data sent to the TX path to be sent over the XCVR --
wire[5 - 1:0] send_ts2_out;
wire send_tds_out;
...
wire pll_areset;
wire rx_locktodata;
wire[5 - 1:0] rx_analogreset;
wire[5 - 1:0] rx_digitalreset;
reg[5 - 1:0] freq_lock_d1;
reg[5 - 1:0] freq_lock_d2;
reg[5 - 1:0] freq_lock_d3;
reg link_d1;
reg link_d2;
reg link_d3;
reg freqlock_lost
...
/*CALL*/
 ...
 five_lane_seriallite_slite2_phy /* vx2 no_prefix */   slite2_phy(.trefclk(tx_coreclock),
// input
