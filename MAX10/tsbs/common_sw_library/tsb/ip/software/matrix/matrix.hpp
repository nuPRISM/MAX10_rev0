...
{
    safe_print(std::cout << pErrMsg << std::endl);
    safe_print(std::cout << "Ending Program via Exit!!!\n");
    exit(1);
}
...
matrixT::realloc (size_t row, size_t col)
{
   ...
   size_t colSize = matrix_min(_m->Col,col) * sizeof(T);
   size_t minRow = matrix_min(_m->Row,row);

   for (size_t i=0; i < minRow; i++)
	   memmove( m1->Val[i], _m->Val[i], colSize);
...
matrixT::operator () (size_t row, size_t col) _THROW_MATRIX_ERROR
{
   ...
   {
	   safe_print(std::cout << "Row: " << row << "M->row: " << _m->Row << "col: " << col << "M->col: " << _m->Col << std::endl);
...
{
    if (_m->Refcnt > 1) clone();   
    size_t row = matrix_min(_m->Row,_m->Col);
    _m->Row = _m->Col = row;
