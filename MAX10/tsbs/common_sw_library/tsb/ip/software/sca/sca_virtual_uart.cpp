/*
 * sca_virtual_uart.cpp
 *
 *  Created on: Feb 7, 2014
 *      Author: yairlinn
 */

#include "sca_virtual_uart.h"
#include "basedef.h"
#include <sstream>
#include <iostream>
#include <string>
...
unsigned long long sca_virtual_uart::read_control_reg(unsigned long address,
		...
		int* errorptr)
{
	 if (!is_valid_secondary_uart(secondary_uart_address)) {
		...
		return this->sca_read(address);
};

void sca_virtual_uart::write_control_reg(unsigned long address,
		...
		int* errorptr)
{
	...
	this->sca_write(address,data);
};



sca_virtual_uart::sca_virtual_uart() :
	virtual_uart_register_file(),
	sca_driver() {
	
	
		default_register_descriptions[0x001] = "Config1"                                        ;
		default_register_descriptions[0x002] = "Config2"                                        ;
		default_register_descriptions[0x003] = "ChanTest1"                                             ;
		default_register_descriptions[0x004] = "ChanTest2"                                           ;
