...
/* See documentation in header file. */
FRESULT ini_parse(const char* filename,
              ...
              void* user)
{
    ...
    ferror = f_open(&file, filename, FA_OPEN_EXISTING | FA_READ);
    if (ferror != FR_OK)
    {
    	xprintf("[ini_parse]: Error: could not open ini file: %s\n",filename);
        return ferror;
    }
    ...
    if (ferror != FR_OK)
    {
    	xprintf("[ini_parse]: Error: could not close ini file: %s\n",filename);
        return ferror;
    }
    return FR_OK;
}
