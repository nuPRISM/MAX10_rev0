...
template <class ELT> CBufferT <ELT> :: CBufferT(const ELT * pcsz, int length) : CBufferRefT <ELT> (0, length)
{
	...
	CBufferRefT <ELT> :: m_pRef = m_pBuffer = (ELT *) my_mem_malloc(sizeof(ELT) * m_nMaxLength);
	memmove(m_pBuffer, pcsz, sizeof(ELT) * CBufferRefT <ELT> :: m_nSize);
...
template <class ELT> void CBufferT <ELT> :: Append(const ELT * pcsz, int length, int eol)
{
	...
	{
		CBufferRefT <ELT> :: m_pRef = m_pBuffer = (ELT *) my_mem_realloc(m_pBuffer, sizeof(ELT) * nNewLength);
		m_nMaxLength = nNewLength;
	}

	// Append
	memmove(m_pBuffer + CBufferRefT <ELT> :: m_nSize, pcsz, sizeof(ELT) * length);
...
template <class ELT> void CBufferT <ELT> :: Release()
{
	...
	if(pBuffer != 0) my_mem_free(pBuffer);
}
...
{
	if(m_pBuffer != 0) my_mem_free(m_pBuffer);
}
...
{
	if(tstring != 0) my_mem_free(tstring);
}
