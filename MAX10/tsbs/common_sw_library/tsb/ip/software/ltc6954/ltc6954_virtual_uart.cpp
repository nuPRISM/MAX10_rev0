/*
 * ltc6954_virtual_uart.cpp
 *
 *  Created on: Feb 7, 2014
 *      Author: yairlinn
 */

#include "ltc6954_virtual_uart.h"
#include "basedef.h"
#include <sstream>
#include <iostream>
#include <string>
...
unsigned long long ltc6954_virtual_uart::read_control_reg(unsigned long address,
		...
		int* errorptr)
{
	 if (!is_valid_secondary_uart(secondary_uart_address)) {
		...
		return this->ltc6954_read(address);
};

void ltc6954_virtual_uart::write_control_reg(unsigned long address,
		...
		int* errorptr)
{
	...
	this->ltc6954_write(address,data);
};



ltc6954_virtual_uart::ltc6954_virtual_uart(unsigned long chipselect, unsigned long id_no) :
	virtual_uart_register_file(),
	 ltc6954_driver(chipselect, id_no) {
