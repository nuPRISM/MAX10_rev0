...
uint32_t spi_calc_cdiv( uint32_t baudrate, uint32_t clock_freq )
{
    uint32_t cdiv;
    cdiv = clock_freq / (baudrate * 2);
    if (((clock_freq / cdiv) >> 1) > baudrate ) cdiv++;
    if ( cdiv == 0 ) cdiv = 1;
    return cdiv - 1;
}
...
void spi_open(spi_t *drv,
		...
		unsigned long CTRL_SETTINGS, unsigned baudrate, unsigned slave_num_mask, uint32_t base_clock_rate,
	    uint32_t        index,
	    struct drv_spi_s *next)
{

    drv->baseaddress = baseaddress;
    drv->base_clock_rate = base_clock_rate;
    drv->channel = 0;         // Initially, the bus is assumed to be free

    // Initialize the core: set baudrate
    spi_set_baudrate( drv, baudrate );
    SPI_CS  ( drv->baseaddress ) = slave_num_mask; //enable CS
...
void spi_set_baudrate( spi_t *  drv, uint32_t baudrate )
{
    ...
    SPI_CDIV( drv->baseaddress ) = spi_calc_cdiv( baudrate , drv->base_clock_rate);
}
...
{
    return (drv->base_clock_rate / (SPI_CDIV( drv->baseaddress ) + 1)) / 2;
}
...
void spi_set_charlen( spi_t *  drv, uint32_t charlen)
{
	...
	//zero out lower 7 bits of ctrl
	ctrl = ctrl >> 7;
	ctrl = ctrl << 7;

	ctrl += (charlen & 0b1111111);
...
uint32_t spi_transceive32( spi_t *  drv, uint32_t val,  unsigned slave_num_mask  )
{

    uint32_t ret = 0;
    SPI_CS  ( drv->baseaddress ) = slave_num_mask | 0x1; //enable CS, use CS[0] as check
...
unsigned long SPI_TransferData(spi_t *  drv, char txSize, char* txBuf, char rxSize, char* rxBuf, unsigned slave_num_mask)
{
    ...
    char rx_data;
    for  (i = 0; i < txSize; i++) {
    	rx_data = spi_transceive32( drv, txBuf[i], slave_num_mask );
    	if (i < rxSize) {
    		rxBuf[i] = rx_data;
    	}
    }
