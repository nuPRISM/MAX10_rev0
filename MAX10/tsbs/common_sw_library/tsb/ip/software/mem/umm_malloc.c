...
#ifdef LINNUX_COMPILE_UMM_MALLOC
// ----------------------------------------------------------------------------
//
...
#ifndef CIRCUMVENT_UMM_MALLOC
  #define umm_realloc realloc
  extern umm_block umm_heap[];
  #define UMM_HEAP_SECTION __attribute__ ((section (".bss")))
#else
  extern umm_block umm_heap[];
  #define UMM_HEAP_SECTION
#endif

  umm_block umm_heap[LINNUX_UMM_MALLOC_HEAP_SIZE_IN_BYTES/sizeof(umm_block)] UMM_HEAP_SECTION;
...
#ifdef CIRCUMVENT_UMM_MALLOC
	free_replacement(ptr);
	return;
#endif
...
#ifdef CIRCUMVENT_UMM_MALLOC
	return malloc_replacement(size);
#endif
           unsigned short int blocks;
...
#ifdef CIRCUMVENT_UMM_MALLOC
	return realloc_replacement(ptr,size);
#endif
  unsigned short int blocks;
...
main() {
  ...
  safe_print(printf( "Size of umm_heap is %i\n", sizeof(umm_heap)       ););
  safe_print(printf( "Size of header   is %i\n", sizeof(umm_heap[0])    ););
  safe_print(printf( "Size of nblock   is %i\n", sizeof(umm_heap[0].header.used.next) ););
  safe_print(printf( "Size of pblock   is %i\n", sizeof(umm_heap[0].header.used.prev) ););
  safe_print(printf( "Size of nfree    is %i\n", sizeof(umm_heap[0].body.free.next)   ););
  safe_print(printf( "Size of pfree    is %i\n", sizeof(umm_heap[0].body.free.prev)   ););
