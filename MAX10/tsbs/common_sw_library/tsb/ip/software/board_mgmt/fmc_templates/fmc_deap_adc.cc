...
tFMCInfo* CreateFMCDeapADC24(void) {
	...
	fmc->board_info.manufacturer_bytes = std::string(str_manufacturer,MAX_STR_LENGTH_FOR_BOARD_MGMT);

	fmc->board_info.product.type_code = 0x03;
	fmc->board_info.product.num_bytes = sizeof(PRODUCT_NAME);
	fmc->board_info.product_bytes =  std::string(str_product,MAX_STR_LENGTH_FOR_BOARD_MGMT);

	fmc->board_info.serial_num.type_code = 0x03;
	fmc->board_info.serial_num.num_bytes = sizeof(SERIAL_NUM);
	fmc->board_info.serial_num_bytes = std::string(str_serial,MAX_STR_LENGTH_FOR_BOARD_MGMT);

	fmc->board_info.part_num.type_code = 0x03;
	fmc->board_info.part_num.num_bytes = sizeof(PART_NUM);
	fmc->board_info.part_num_bytes =std::string(str_part,MAX_STR_LENGTH_FOR_BOARD_MGMT);
