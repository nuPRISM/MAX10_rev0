...
)
{
	int ret;
    INT8U err;
    char ff_cre_mutex_name[OS_EVENT_NAME_SIZE] = "ffCreMutex";
...
//	ret = 1;					/* The initial value of the semaphore must be 1. */
	...
	*sobj = OSMutexCreate(0, &err);			/* uC/OS-II */
	ret = (err == OS_NO_ERR);
	if (!ret) {
		       safe_print(printf("[FatFs ff_cre_syncobj] Error encountered during OSMutexCreate, error = [%d]\n", (int) err));
	        }

		if (ret) {
			  OSEventNameSet(*sobj,(INT8U *)ff_cre_mutex_name,&err);
			  if (err != OS_NO_ERR) {
				        safe_print(printf("Error in initializing name of: ffCreMutex\n"));
			     	}
		}
...
//	ret = 1;					/* uITRON (nothing to do) */
	...
	ret = (err == OS_NO_ERR);
	if (!ret) {
	   printf("[FatFs ff_del_syncobj] Error encountered during OSMutexDel, error = [%d]\n", (int) err);
	}
...
//	ret = (wai_sem(sobj) == E_OK);	/* uITRON */
	...
	ret = (err == OS_NO_ERR);
	if (!ret) {
	  printf("[FatFs ff_req_grant] Error encountered during semaphore pend, error = [%d], timeout = [%d], Process = [%d]\n", (int) err, (int) _FS_TIMEOUT, OSTCBCur->OSTCBPrio);
	}
