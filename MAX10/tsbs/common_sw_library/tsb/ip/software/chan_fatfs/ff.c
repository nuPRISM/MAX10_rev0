...
#define	ABORT(fs, res)		{ fp->flag |= FA__ERROR; alt_printf("[ff.c] ABORT\n");  alt_printf("[ff.c][TS_Sec=%x] ABORT: Process: [%x] Function:[%s] File:[%s] Line:[%x] \n",(unsigned int)c_low_level_system_timestamp_in_secs(),(unsigned int) OSTCBCur->OSTCBPrio,__FUNCTION__,__FILE__,(unsigned int)__LINE__); LEAVE_FF(fs, res); }
...
)
{
	...
	if (res == FR_OK) {
		...
		if (fs->fs_type == FS_FAT32 && fs->fsi_flag) {
			...
			if (disk_write(fs->drv, fs->win, fs->fsi_sector, 1) != RES_OK) {
				return FR_DISK_ERR;
			}
			fs->fsi_flag = 0;
		}
