...
#define JANSSONXX_NUM_SPACES_TO_INDENT 4
...
namespace json {
	...
	public:
	    size_t JANSSONXX_STRSTREAM_FLAGS;
	inline explicit Value(const char* value);
	inline explicit Value(const std::string& value);
	inline explicit Value(bool value);
	inline explicit Value(signed int value);
	inline explicit Value(unsigned int value);
	inline explicit Value(signed short value);
	inline explicit Value(unsigned short value);
	inline explicit Value(signed long value);
	inline explicit Value(unsigned long value);
	inline explicit Value(float value);
	inline explicit Value(double value);

	size_t getJanssonxxStrstreamFlags() const
	{
		return JANSSONXX_STRSTREAM_FLAGS;
	}

	void setJanssonxxStrstreamFlags(size_t janssonxxStrstreamFlags)
	{
		JANSSONXX_STRSTREAM_FLAGS = janssonxxStrstreamFlags;
	}

	Value()
	:_private::ValueBase<_private::Basic>()
	{
		setJanssonxxStrstreamFlags(0);
		//JANSSONXX_STRSTREAM_FLAGS = (JSON_INDENT(JANSSONXX_NUM_SPACES_TO_INDENT) | JSON_ENSURE_ASCII | JSON_PRESERVE_ORDER);
	}

	Value(const _private::Basic& value)
	:_private::ValueBase<_private::Basic>(value)
	{
	}

	Value(const _private::ValueBase<_private::Basic>& value)
	:_private::ValueBase<_private::Basic>(value)
	{
	}
	...
	explicit Value(json_t* json)
	:_private::ValueBase<_private::Basic>(json)
	{
	}
};
